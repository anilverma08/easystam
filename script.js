...
styleFix.innerHTML = `
    ...
    #btn-present, #btn-absent, #btn-halfday, #btn-leave, #send-otp-btn { 
...
window.requestLoginOTP = function() {
    ...
    if (!email) { alert("Kripya pehle Email address enter karein."); return; }
    if (!password) { alert("Kripya login karne ke liye password enter karein."); return; }
    ...
    .catch(err => {
        statusMsg.style.color = "#ef4444";
        statusMsg.innerText = "Error: Connection to system database failed.";
    });
};
...
window.verifyLoginOTP = function() {
    ...
    if (email === localOTPSession.targetEmail && otp === localOTPSession.generatedOTP) {
        ...
        statusMsg.style.color = "#10b981";
        statusMsg.innerText = "Login safal raha!";
        forceOpenDashboard();
    } else {
        statusMsg.style.color = "#ef4444";
        statusMsg.innerText = "Galat OTP! Kripya sahi code daalein.";
    }
};
...
window.requestRecoveryOTP = function() {
    ...
    .then(cloudData => {
        if (cloudData !== null || email === 'anil@example.com' || email === localStorage.getItem('last_registered_email')) {
            ...
            .catch(() => {
                statusMsg.style.color = "#ef4444";
                statusMsg.innerText = "Failed to send recovery OTP. Check connection.";
            });
        } else {
            statusMsg.style.color = "#ef4444";
    ...
    .catch(() => {
        statusMsg.style.color = "#ef4444";
        statusMsg.innerText = "Database connection error.";
    });
};
...
window.verifyRecoveryOTP = function() {
    ...
    if (inputOtp === recoveryOTPSession.generatedOTP) {
        ...
        statusMsg.style.color = "#ef4444";
        statusMsg.innerText = "Incorrect OTP code. Please enter valid code.";
    }
};
...
window.handleRecoverySubmit = function(event) {
    ...
    .then(() => {
        toggleAuthScreens('login');
        const mainStatus = document.getElementById('auth-status-msg');
        if (mainStatus) {
            mainStatus.style.color = "#10b981";
            mainStatus.innerText = "Password Recovery Success! Please Login using your new credentials.";
        }
...
function toggleThreeDotMenu(event) {
    ...
    if (dropdownContainer) {
        if (dropdownContainer.style.display === 'block') { dropdownContainer.style.display = 'none'; dropdownContainer.style.pointerEvents = 'none'; }
        else { dropdownContainer.style.display = 'block'; dropdownContainer.style.pointerEvents = 'auto'; }
    }
}
